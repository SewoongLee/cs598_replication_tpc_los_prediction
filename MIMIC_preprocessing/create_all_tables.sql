...
-- change the paths to those in your local computer using find and replace for 'D:/datasets/mimic-iv-0.4/'.
...
\copy (select subject_id as uniquepid, hadm_id as patienthealthsystemstayid, stay_id as patientunitstayid, hospital_expire_flag as actualhospitalmortality, los as actualiculos from ld_labels as l where l.stay_id in (select * from ld_timeseries_patients) order by l.stay_id) to 'D:/datasets/mimic-iv-0.4/labels.csv' with csv header
\copy (select * from ld_flat as f where f.patientunitstayid in (select * from ld_timeseries_patients) order by f.patientunitstayid) to 'D:/datasets/mimic-iv-0.4/flat_features.csv' with csv header
\copy (select * from ld_timeserieslab as tl order by tl.patientunitstayid, tl.labresultoffset) to 'D:/datasets/mimic-iv-0.4/timeserieslab.csv' with csv header
\copy (select * from ld_timeseries as t order by t.patientunitstayid, t.chartoffset) to 'D:/datasets/mimic-iv-0.4/timeseries.csv' with csv header